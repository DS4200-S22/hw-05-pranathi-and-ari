...
// Define color scale
const color = d3.scaleOrdinal().domain([ 'setosa', 'versicolor', 'virginica' ])
...
d3.csv('data/iris.csv').then((data) => {
  ...
  // Scatterplot1
  {
    ...
    brush1 = d3.brush().extent([ [ 0, 0 ], [ width, height ] ]).on('brush', updateChart1)
        .on('start', clear);
    // Add brush1 to svg1
  ...
  // Scatterplot 2 (show Sepal width on x-axis and Petal width on y-axis)
  {
    ...
    brush2 = d3.brush().extent([ [ 0, 0 ], [ width, height ] ]).on('brush', updateChart2)
        .on('start', clear);
    // Add brush2 to svg2
  ...
  {
    // Bar chart code here 
    // Hardcoded barchart data for count of Iris species
    barData = [
...

    {
      ...
      // Find max y
      maxY3 = d3.max(barData, function (d) {return d.count;});
  ...
  function updateChart1(brushEvent) {
    ...
    // Give bold outline to all points in Scatterplot2 corresponding to points within the
    // brush region in Scatterplot1
  ...
  function updateChart2(brushEvent) {
    ...
    // Give bold outline to all points within the brush region in Scatterplot2 & collected names
    // of brushed species
    myCircles2.classed('selected', (d) => {
    ...
    // Give bold outline to all points in Scatterplot1 corresponding to points within the brush
    // region in Scatterplot2
    myCircles1.classed('selected', (d) => {
      return isBrushed(extent, x2(d[xKey2]), y2(d[yKey2]));
    });
    // Give bold outline to all bars in bar chart with corresponding to species selected by
    // Scatterplot2 brush
    myBars1.classed('selected', (d) => {
  ...
  function isBrushed(brush_coords, cx, cy) {
    ...
    let x0 = brush_coords[0][0],
        ...
        y0 = brush_coords[0][1],
        y1 = brush_coords[1][1];
    return x0 <= cx && cx <= x1 && y0 <= cy && cy <= y1; // This return TRUE or FALSE depending on
                                                         // if the points is in the selected area
  }
});
